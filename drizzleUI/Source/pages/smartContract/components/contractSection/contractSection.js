...
class ContractSection extends React.Component {
	renderContractDropDown = () => {
		const { contract, contracts, onChange } = this.props;
	...
	renderDeploymentArea = () => {
		const { context, contract } = this.props;

		const contractData = context.drizzle.contracts[contract].abi.find(
			(el) => el.type === "constructor",
		);

		return (
			<React.Fragment>
				{contractData.inputs.map((input, index) => (
					<div key={index} className="deployment area">
						<Label
							text={input.name}
							style={{
				...
				<Button text="Deploy" className="deploy button" />
			</React.Fragment>
		);
	};
