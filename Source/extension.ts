...
export async function activate(context: ExtensionContext) {
	...
	const refresh = commands.registerCommand(
		"azureBlockchainService.refresh",
		(element) => {
	...
	const copyRPCEndpointAddress = commands.registerCommand(
		...
		async (viewItem: ConsortiumView) => {
			await tryExecute(() =>
				AzureBlockchain.copyRPCEndpointAddress(viewItem),
			);
		},
	);
	...
	const startGanacheServer = commands.registerCommand(
		"azureBlockchainService.startGanacheServer",
		async () => {
	...
	const stopGanacheServer = commands.registerCommand(
		"azureBlockchainService.stopGanacheServer",
		async () => {
	...
	const newSolidityProject = commands.registerCommand(
		"truffle.newSolidityProject",
		async () => {
	...
	const deployContracts = commands.registerCommand(
		...
		async () => {
			await tryExecute(() =>
				TruffleCommands.deployContracts(consortiumTreeManager),
			);
		},
	);
	...
	const createConsortium = commands.registerCommand(
		"azureBlockchainService.createConsortium",
		async () => {
	...
	const connectConsortium = commands.registerCommand(
		...
		async () => {
			await tryExecute(() =>
				ConsortiumCommands.connectConsortium(consortiumTreeManager),
			);
		},
	);
	...
	const disconnectConsortium = commands.registerCommand(
		...
		async (viewItem: ConsortiumView) => {
			await tryExecute(() =>
				ConsortiumCommands.disconnectConsortium(
	...
	const viewGasEstimates = commands.registerCommand(
		"remix.viewGasEstimates",
		async () => {
	...
	const pushCurrentLedgerEvents = commands.registerCommand(
		"azureBlockchainService.pushCurrentLedgerEvents",
		async () => {
			await tryExecute(() =>
				runWithCurrentOpenFile(WestlakeCommands.showLedgerEventsDialog),
			);
		},
	);
	...
	const pushLedgerEvents = commands.registerCommand(
		...
		async (uri: Uri) => {
			await tryExecute(() =>
				WestlakeCommands.showLedgerEventsDialog(uri),
			);
		},
	);
	//#endregion
	...
	const generateMicroservicesWorkflows = commands.registerCommand(
		...
		async (filePath: Uri | undefined) => {
			await tryExecute(() =>
				LogicAppCommands.generateMicroservicesWorkflows(filePath),
			);
		},
	);
	...
	const generateDataPublishingWorkflows = commands.registerCommand(
		...
		async (filePath: Uri | undefined) => {
			await tryExecute(() =>
				LogicAppCommands.generateDataPublishingWorkflows(filePath),
			);
		},
	);
	...
	const generateEventPublishingWorkflows = commands.registerCommand(
		...
		async (filePath: Uri | undefined) => {
			await tryExecute(() =>
				LogicAppCommands.generateEventPublishingWorkflows(filePath),
			);
		},
	);
	...
	const generateReportPublishingWorkflows = commands.registerCommand(
		...
		async (filePath: Uri | undefined) => {
			await tryExecute(() =>
				LogicAppCommands.generateReportPublishingWorkflows(filePath),
			);
		},
	);
	//#endregion
