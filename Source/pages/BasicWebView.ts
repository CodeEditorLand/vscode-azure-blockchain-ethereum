...
export abstract class BasicWebView {
	...
	public async checkAndShow(): Promise<void> {
		const showOnStartup = this.context.globalState.get(
			this.config.showOnStartup,
		);
		...
		if (showOnStartup === undefined) {
			this.context.globalState.update(
				this.config.showOnStartup,
				await this.setShowOnStartupFlagAtFirstTime(),
			);
		}
	...
	protected async getHtmlForWebview(): Promise<string> {
		if (this.panel) {
			const rootPath = this.panel.webview
	...
	protected async receiveMessage(message: {
		...
		switch (message.command) {
			...
			case "openLink":
				Telemetry.sendEvent(
					Constants.telemetryEvents.webPages.action,
					message,
				);
				break;
			case "notification":
