...
export class ChangelogPage extends BasicWebView {
	...
	public async checkAndShow(): Promise<void> {
		...
		if (
			storedVersion &&
			semver.gte(storedVersion, Constants.extensionVersion)
		) {
			return;
		}
	...
	protected async getHtmlForWebview(): Promise<string> {
		if (this.panel) {
			const resourcePath = this.panel.webview
				.asWebviewUri(this.rootPath)
				.toString();
			const html = await fs.readFile(this.config.path, "utf8");
			const content = await fs.readFile(
				Constants.webViewPages.changelog.changelogPath,
				"utf8",
			);
			const htmlContent = (await commands.executeCommand(
				"markdown.api.render",
				content,
			)) as string;

			return html
				.replace(/{{root}}/g, resourcePath)
