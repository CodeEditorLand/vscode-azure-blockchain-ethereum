...
export namespace GanacheCommands {
	const ganacheOutputChannel: OutputChannel = window.createOutputChannel(
		Constants.outputChannel.ganacheCommands,
	);
	...
	export async function startGanacheCmd(): Promise<void> {
		...
		window.showInformationMessage(
			Constants.ganacheCommandStrings.serverSuccessfullyRunning,
		);
	}
	...
	export async function startGanacheServer(): Promise<void> {
		if (!server && (await required.checkRequiredApps())) {
			...
			setCommandContext(CommandContext.IsGanacheRunning, true);
		}
	}
