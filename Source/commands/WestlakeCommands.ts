...
export namespace WestlakeCommands {
	...
	export async function selectWestlakeConsortium(
	...
	export async function showLedgerEventsDialog(uri: Uri): Promise<void> {
		...
		const target = await window.showQuickPick(ledgerEventsDestination, {
			placeHolder: Constants.placeholders.selectLedgerEventsDestination,
			ignoreFocusOut: true,
		});
