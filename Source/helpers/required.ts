...
export namespace required {
	...
	export async function checkAllApps(): Promise<boolean> {
		...
		if (!valid) {
			...
			const details =
				Constants.informationMessage.seeDetailsRequirementsPage;
			window
				.showErrorMessage(
					`${message}. ${details}`,
					Constants.informationMessage.detailsButton,
				)
				.then((answer) => {
					if (answer) {
	...
	export async function checkApps(...apps: RequiredApps[]): Promise<boolean> {
		...
		if (!valid) {
			Telemetry.sendEvent(
				Constants.telemetryEvents.failedToCheckRequiredApps,
			);
			const message =
	...
	export async function checkAppsSilent(
		...apps: RequiredApps[]
	): Promise<boolean> {
	...
	export async function getHdWalletProviderVersion(): Promise<string> {
		try {
			const data =
				fs.readFileSync(
					path.join(getWorkspaceRoot()!, "package-lock.json"),
					null,
				) ||
				fs.readFileSync(
	...
	export async function getExactlyVersions(
		...
		Output.outputLine(
			"",
			`Get version for required apps: ${apps.join(",")}`,
		);
	...
	export async function getTruffleVersion(): Promise<string> {
		const requiredVersion =
			Constants.requiredVersions[RequiredApps.truffle];
		...
		const localVersion = (
			await tryExecuteCommand(
				...
				`npm list --depth 0 truffle@${majorVersion}`,
			)
	...
	export async function getGanacheVersion(): Promise<string> {
		const requiredVersion =
			Constants.requiredVersions[RequiredApps.ganache];
		const minRequiredVersion =
			typeof requiredVersion === "string"
				? requiredVersion
				: requiredVersion.min;
		...
		return (
			(localVersion && localVersion[1]) ||
			getVersion(RequiredApps.ganache, "--version", /v(\d+.\d+.\d+)/)
		);
	}
	...
	export async function installNpm(): Promise<void> {
		try {
			await installUsingNpm(
				RequiredApps.npm,
				Constants.requiredVersions[RequiredApps.npm],
			);
		} catch (error) {
			Telemetry.sendException(error);
			Output.outputLine(
				Constants.outputChannel.requirements,
				error.message,
			);
		}

		currentState.npm = await createRequiredVersion(
			RequiredApps.npm,
			getNpmVersion,
		);
	}
	...
	export async function isHdWalletProviderRequired(): Promise<boolean> {
		try {
			const truffleConfigPath =
				TruffleConfiguration.getTruffleConfigUri();
			const config = new TruffleConfiguration.TruffleConfig(
				truffleConfigPath,
			);
	...
	async function installUsingNpm(
		...
		if (workspaceRoot === undefined && scope === Scope.locally) {
			const error = new Error(
				Constants.errorMessageStrings.WorkspaceShouldBeOpened,
			);
			Telemetry.sendException(error);
			throw error;
		}
