...
export namespace required {
	 ...
	 * Node.js, npm, git
	 */
	export async function checkRequiredApps(
		message?: string,
	): Promise<boolean> {
		const versions = await getAllVersions();
	...
	export async function getAllVersions(): Promise<IRequiredVersion[]> {
		...
		currentState.ganache =
			...
			(await createRequiredVersion(
				"ganache",
				getGanacheVersion,
				CommandContext.GanacheIsAvailable,
			));
	...
	export function showRequiredAppsMessage(message?: string): void {
		...
		timeoutID = setTimeout(async () => {
			try {
				message =
					message ||
					Constants.informationMessage.invalidRequiredVersion;

				window.showErrorMessage(
					`${message} ${Constants.informationMessage.seeDetailsRequirementsPage}`,
				);
			} catch (e) {
				// ignore
			}
	...
	export async function installTruffle(): Promise<void> {
		try {
			await installUsingNpm(
				"truffle",
				Constants.requiredVersions.truffle,
			);
		} catch (error) {
			// ignore
		}
	...
	export async function installGanache(): Promise<void> {
		try {
			await installUsingNpm(
				"ganache-cli",
				Constants.requiredVersions.ganache,
			);
		} catch (error) {
			// ignore
		}
