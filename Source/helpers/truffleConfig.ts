...
export namespace TruffleConfiguration {
	...
	const ignore = ["build/**/*", "out/**/*", "dist/**/*", "test/**/*"];

	const ignoreWorkspace = [...ignore, "node_modules/**/*"];
	...
	export class TruffleConfig {
		...
		public async writeAST(): Promise<void> {
			return fs.writeFile(
				this.filePath,
				generate(this.ast as ESTree.Node, { comments: true }),
			);
		}
		...
		public async getNetworks(): Promise<INetwork[]> {
			...
			if (moduleExports.node) {
				...
				if (rightExpression.type === "ObjectExpression") {
					const networksNode = findProperty(
						rightExpression,
						"networks",
					);
					...
					if (
						...
						networksNode.value.properties.forEach(
	...
	function astToNetworkOptions(
		...
		if (
			...
			typeof consortiumId.value.value === "number"
		) {
	...
	function networkOptionsToAst(network: INetwork): ESTree.ObjectExpression {
		...
		if (options.gas !== undefined) {
			obj.properties.push(
				generateProperty("gas", generateLiteral(options.gas)),
			);
		}
		...
		if (options.consortium_id !== undefined) {
			obj.properties.push(
				generateProperty(
					"consortium_id",
					generateLiteral(options.consortium_id),
				),
			);
		}
