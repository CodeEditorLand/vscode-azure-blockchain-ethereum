...
export namespace TruffleConfiguration {
	...
	export class TruffleConfig {
		...
		public writeAST(): void {
			return fs.writeFileSync(
				this.filePath,
				generate(this.ast, { comments: true }),
			);
		}
		...
		public getNetworks(): INetwork[] {
			...
			if (moduleExports) {
				...
				if (
					...
					Telemetry.sendEvent(
						"TruffleConfig.getNetworks.objectExpression",
					);
		...
		public setNetworks(network: INetwork): void {
			...
			if (moduleExports) {
				Telemetry.sendEvent(
					"TruffleConfig.setNetworks.moduleExportsTrue",
				);
				let networksNode = findProperty(moduleExports, "networks");
				if (!networksNode) {
					Telemetry.sendEvent(
						"TruffleConfig.setNetworks.noNetworksNode",
					);
				...
				if (networksNode.value.type === "ObjectExpression") {
					Telemetry.sendEvent(
						"TruffleConfig.setNetworks.objectExpression",
					);
					const isExist = findProperty(
						networksNode.value,
						network.name,
					);
					if (isExist) {
						Telemetry.sendException(
							new Error(
								Constants.errorMessageStrings.NetworkAlreadyExist(
									Telemetry.obfuscate(network.name),
								),
							),
						);
						throw new Error(
						...
						Telemetry.sendEvent(
							"TruffleConfig.setNetworks.addNetworkNode",
						);
						const networkNode = generateProperty(
							network.name,
							generateObjectExpression(),
						);
		...
		public importPackage(variableName: string, packageName: string): void {
			const packageRequired: IFound = walk.findNodeAt(
				this.ast,
				null,
				null,
				isVarDeclaration(variableName),
			);
			if (!packageRequired) {
		...
		public isHdWalletProviderDeclared(): boolean {
			try {
				const moduleExports = walk.findNodeAt(
					this.ast,
					null,
					null,
					isHdWalletProviderDeclaration,
				);
	...
	function isHdWalletProviderDeclaration(
		nodeType: string,
		node: ESTree.Node,
	): boolean {
		if (nodeType === "NewExpression") {
			...
			return (
				node.name ===
				Constants.truffleConfigRequireNames.hdwalletProvider
			);
		}
	...
	function getModuleExportsObjectExpression(
		ast: ESTree.Node,
	): ESTree.ObjectExpression | void {
		...
		if (
			moduleExports &&
			moduleExports.node.type === "ExpressionStatement"
		) {
			const rightExpression = (
				moduleExports.node.expression as ESTree.AssignmentExpression
			).right;
	...
	async function getTruffleMetadata(): Promise<IConfiguration> {
		const truffleConfigTemplatePath = path.join(
			__dirname,
			"checkTruffleConfigTemplate.js",
		);

		const result = await tryExecuteCommandInFork(
			getWorkspaceRoot()!,
			truffleConfigTemplatePath,
		);
		const truffleConfigObject = result.messages!.find(
			(message) => message.command === "truffleConfig",
		);
	...
	function isModuleExportsExpression(
		...
		if (
			...
			if (
				...
				(node.expression.left.property.type === "Literal" &&
					node.expression.left.property.value === "module")
			) {
				return true;
			}
		}
	...
	function isVarDeclaration(
		varName: string,
	): (nodeType: string, node: ESTree.Node) => boolean {
	...
	function astToNetworkOptions(
		...
		if (
			...
			typeof port.value.value === "number"
		) {
		...
		if (
			...
			typeof host.value.value === "string"
		) {
		...
		if (
			...
			typeof websockets.value.value === "boolean"
		) {
		...
		if (
			...
			typeof gasPrice.value.value === "number"
		) {
		...
		if (
			...
			typeof from.value.value === "string"
		) {
		...
		if (
			...
			typeof skipDryRun.value.value === "boolean"
		) {
		...
		if (
			...
			typeof timeoutBlocks.value.value === "number"
		) {
	...
	function networkOptionsToAst(network: INetwork): ESTree.ObjectExpression {
		...
		if (options.network_id !== undefined) {
			obj.properties.push(
				generateProperty(
					"network_id",
					generateLiteral(options.network_id),
				),
			);
		}
		...
		if (options.port !== undefined) {
			obj.properties.push(
				generateProperty("port", generateLiteral(options.port)),
			);
		}

		if (options.host !== undefined) {
			obj.properties.push(
				generateProperty("host", generateLiteral(options.host)),
			);
		}
		...
		if (options.websockets !== undefined) {
			obj.properties.push(
				generateProperty(
					"websockets",
					generateLiteral(options.websockets),
				),
			);
		}
		...
		if (options.gasPrice !== undefined) {
			obj.properties.push(
				generateProperty("gasPrice", generateLiteral(options.gasPrice)),
			);
		}

		if (options.from !== undefined) {
			obj.properties.push(
				generateProperty("from", generateLiteral(options.from)),
			);
		}
		...
		if (options.skipDryRun !== undefined) {
			obj.properties.push(
				generateProperty(
					"skipDryRun",
					generateLiteral(options.skipDryRun),
				),
			);
		}
		...
		if (options.timeoutBlocks !== undefined) {
			obj.properties.push(
				generateProperty(
					"timeoutBlocks",
					generateLiteral(options.timeoutBlocks),
				),
			);
		}
		...
		if (options.provider !== undefined) {
			obj.properties.push(
				generateProperty(
					"provider",
					hdWalletProviderToAst(options.provider),
				),
			);
		}
	...
	function isMnemonicNode(
		node: ESTree.Literal | ESTree.NewExpression,
	): boolean {
		return (
	...
	function astToHDWalletProvider(node: ESTree.NewExpression): IProvider {
		...
		const mnemonicNode = node.arguments[0] as ESTree.NewExpression &
			ESTree.Literal;
		const mnemonicFilePathNode =
			mnemonicNode &&
			mnemonicNode.arguments &&
			(mnemonicNode.arguments[0] as ESTree.Literal);

		if (isMnemonicNode(mnemonicNode)) {
			provider.mnemonic = mnemonicNode.value as string;
		} else if (isMnemonicNode(mnemonicFilePathNode)) {
			provider.mnemonic = MnemonicRepository.getMnemonic(
				mnemonicFilePathNode.value as string,
			);
		}
	...
	function hdWalletProviderToAst(provider: IProvider): ESTree.NewExpression {
		return {
			arguments: [
				generateFsReadExpression(
					"fs.readFileSync",
					(provider.mnemonic || "").replace(/\\/g, "\\\\"),
				),
	...
	function jsonToConfiguration(truffleConfig: {
		[key: string]: any;
	}): IConfiguration {
		const {
			contracts_directory,
		...
		node.properties.forEach((property: ESTree.Property) => {
			if (property.key.type === "Identifier") {
				networks.push({
					...
					options: astToNetworkOptions(
						property.value as ESTree.ObjectExpression,
					),
				});
			}
	...
	function generateFsReadExpression(
		operator: string,
		args: string,
