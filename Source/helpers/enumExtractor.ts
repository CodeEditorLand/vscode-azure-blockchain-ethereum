...
export function extractEnumsInfoSafe(
	contractName: string,
	ast: { [key: string]: any },
): EnumStorage {
	try {
...
export function extractEnumsInfo(
	...
	if (enumNodes.length > 0) {
		...
		methods.forEach((method) => {
			method.parameters.parameters
				.filter((parameter: any) =>
					enumNodes.some(
						(enumItem) => enumItem.name === parameter.typeName.name,
					),
				)
				.forEach((parameter: any) => {
					const enm = enumNodes.find(
						(enumItem) => enumItem.name === parameter.typeName.name,
					)!;
					...
					const enumItems = enm.members.map((enumMember) => ({
						name: enumMember.name,
						value: enumMember.id - offset,
					}));
		...
		const contractMembers = GetEnumMembers(
			"VariableDeclaration",
			rootNode,
			enumNames,
		);
		contractMembers.forEach((contractMember) => {
			const enumItemsWithoutOffset = enumNodes.find(
				(enumNode) => enumNode.name === contractMember.typeName.name,
			)!;
			const offset = enumItemsWithoutOffset.members[0].id;
			result.fields[contractMember.name] =
				enumItemsWithoutOffset.members.map(
					(enumWithoutOffset: any) => ({
...
function ContainsEnum(node: INode, enumNames: string[]): boolean {
	return node.parameters.parameters.some(
		(parameter: any) =>
			enumNames.includes(parameter.typeName.name) &&
			parameter.nodeType === "VariableDeclaration",
	);
}
...
function GetEnumMembers(
	...
	return node.nodes.filter(
		(subnode: INode) =>
			subnode.nodeType === nodeType &&
			enumNames.includes(subnode.typeName.name),
	);
}

function GetMethodsWithEnum(
	nodeType: string,
	node: INode,
	enumNames: string[],
): INode[] {
