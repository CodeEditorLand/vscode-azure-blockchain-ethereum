...
export async function killPid(pid: number = Number.NaN): Promise<void> {
	if (isNaN(pid)) {
		return;
	}

	return tryExecuteCommand(undefined, killPidCommand(pid)).then(
		() => undefined,
	);
}
...
function parsePid(stdout: string): number {
	...
	return pid ? Number.parseInt(pid[0].trim(), 10) : Number.NaN;
}
