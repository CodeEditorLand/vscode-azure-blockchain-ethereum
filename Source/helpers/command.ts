...
export async function executeCommand(
	...
	if (result.code !== 0) {
		Telemetry.sendException(
			new Error("commands.executeCommand.resultWithIncorrectCode"),
		);
		throw new Error(
			Constants.executeCommandMessage.failedToRunCommand(
				commands.concat(" ", ...args.join(" ")),
			),
		);
	}
...
export async function tryExecuteCommand(
	...
	const { result } = tryExecuteCommandAsync(
		workingDirectory,
		true,
		commands,
		...args,
...
export async function executeCommandInFork(
	...
	Telemetry.sendEvent(
		"command.executeCommandInFork.tryExecuteCommandInForkWasStarted",
	);
	...
	if (result.code !== 0) {
		Telemetry.sendException(
			new Error("commands.executeCommandInFork.resultWithIncorrectCode"),
		);
		throw new Error(
			Constants.executeCommandMessage.failedToRunScript(modulePath),
		);
	}
...
export async function tryExecuteCommandInFork(
	...
	const { result } = tryExecuteCommandInForkAsync(
		workingDirectory,
		false,
		modulePath,
		...args,
		...
		(resolve: (res: any) => void, reject: (error: Error) => void): void => {
			...
			childProcess.on("message", (message: IForkMessage) => {
				if (message.batch) {
					...
					batches[message.command][message.batch.index] =
						message.batch.message;
					if (message.batch.done) {
						messages.push({
							command: message.command,
							message: batches[message.command].join(""),
						});
					}
				} else {
