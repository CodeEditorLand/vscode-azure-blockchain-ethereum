...
export function mapNetworkName(networkName: string) {
	const prefix = networkName
		.replace(
			new RegExp(`(${Constants.treeItemData.service.azure.prefix})_(.*)`),
			"$1",
		)
		.replace(
			new RegExp(
				`(${Constants.treeItemData.service.infura.prefix})_(.*)`,
			),
			"$1",
		)
		.replace(
			new RegExp(`(${Constants.treeItemData.service.local.prefix})_(.*)`),
			"$1",
		)
