...
import {
	...
	OpenZeppelinProjectJsonService,
	OpenZeppelinService,
} from "../services";
import {
...
export async function tryGetCurrentOpenZeppelinVersionAsync(): Promise<string> {
	try {
		...
		if (ex instanceof InvalidOpenZeppelinVersionException) {
			...
			const latestVersion =
				await OpenZeppelinService.getLatestOpenZeppelinVersionAsync();
			...
			await OpenZeppelinService.setVersionAsync(
				latestVersion,
				error.location,
			);
...
export async function upgradeOpenZeppelinUserSettingsAsync() {
	...
	return userSettings.updateConfigurationAsync(
...
export async function upgradeOpenZeppelinContractsAsync(): Promise<void> {
	await window.withProgress(
		...
		async () => {
			...
			await OpenZeppelinService.updateOpenZeppelinContractsAsync(
				manifest,
			);
...
export async function defineContractRequiredParameters(): Promise<void> {
	const assetsWithNotSpecifiedRequiredParameters =
		await getAssetsWithNotSpecifiedRequiredParameters();
	...
	if (
		await showConfirmationDialog(
			Constants.openZeppelin.specifyContractParameters,
		)
	) {
...
function getManifestMetadata(version: string): IOZMetadata {
	return require(
		`../services/openZeppelin/manifest-${version}.json`,
	) as IOZMetadata;
}
...
async function setContractParameters(assets: IOZAsset[]): Promise<void> {
	...
	try {
				...
				for (const parameter of contractParameters) {
					if (!parameter.value) {
						parameter.value = await showInputBox({
							...
							validateInput: (v) =>
								validateSolidityType(v, parameter.type),
						});
					}
				}
			}
		...
		await OpenZeppelinProjectJsonService.storeProjectJsonAsync(
			projectMetadata,
		);
	} catch (error) {
		if (
			await showConfirmationDialog(
				Constants.openZeppelin.saveSpecifiedParameters,
			)
		) {
			await OpenZeppelinProjectJsonService.addAssetsToProjectJsonAsync(
				assets,
...
async function getAssetsWithNotSpecifiedRequiredParameters() {
	...
	return currentAssets.reduce((assets, asset) => {
		const manifestAsset = manifestAssets.find(
			(assetFromManifest) => assetFromManifest.id === asset.id,
		);
		if (manifestAsset && !!manifestAsset.requiredParameters) {
			if (!!asset.requiredParameters) {
				for (const [contractName, contractParameters] of Object.entries(
					...
					const manifestContractParameters =
						manifestAsset.requiredParameters[contractName];
					if (manifestContractParameters) {
						for (const parameter of contractParameters) {
							if (parameter.value === undefined) {
								const param = manifestContractParameters.find(
									(manifestContractParameter) =>
										manifestContractParameter.name ===
											parameter.name &&
										manifestContractParameter.type ===
											parameter.type,
								);
								if (param) {
									assets.push(asset);
