...
export async function showInputBox(options: InputBoxOptions): Promise<string> {
	...
	if (result === undefined) {
		Telemetry.sendEvent("userInteraction.showInputBox.userCancellation", {
...
export async function showQuickPick<T extends QuickPickItem>(
	...
	if (result === undefined) {
		Telemetry.sendEvent("userInteraction.showQuickPick.userCancellation", {
			placeHolder: options.placeHolder || "",
		});
		throw new CancellationEvent();
	}
...
export async function showConfirmPaidOperationDialog() {
	...
	if (
		answer.toLowerCase() !==
		Constants.confirmationDialogResult.yes.toLowerCase()
	) {
		Telemetry.sendEvent(
...
export async function saveTextInFile(
	...
	if (!file) {
		Telemetry.sendEvent("userInteraction.saveTextInFile.userCancellation", {
			label: "fileNotSelected",
		});
		throw new CancellationEvent();
	}
...
export async function showNotification(
	options: Notification.IShowNotificationOptions,
): Promise<void> {
...
export async function showIgnorableNotification(
	message: string,
	fn: () => Promise<any>,
): Promise<void> {
	const ignoreNotification = workspace
		.getConfiguration("azureBlockchainService")
		.get("ignoreLongRunningTaskNotification");
