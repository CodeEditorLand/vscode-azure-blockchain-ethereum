...
import type {
	IGetStorageAccountDto,
	IStorageAccountDto,
} from "../ARMBlockchain";
import type { ICreateStorageAccountDto } from "../ARMBlockchain/AzureDto/StorageAccount/CreateStorageAccountDto";
...
export class StorageAccountResourceExplorer extends AzureResourceExplorer {
	public async getFileBlobUrls(
		...
		const client = await this.getStorageAccountClient(
			subscriptionId,
			resourceGroup,
		);
		const storageAccountName = await this.getStorageAccountName();

		await this.createStorageAccountIfDoesNotExist(
			client,
			client.location,
			storageAccountName,
		);
		...
		await this.createContainerIfDoesNotExist(
			storageAccountName,
			containerName,
			sas,
		);
		...
		return window.withProgress(
			{
				...
				title: Constants.statusBarMessages.createBlobs,
			},
			async () =>
				await Promise.all(
	...
	public async deleteBlobs(
		...
		await window.withProgress(
			{
				location: ProgressLocation.Window,
				title: Constants.statusBarMessages.deleteBlobs,
			},
			async () => {
				await Promise.all(
					fileUrls.map((url) =>
						this.deleteBlob(new URL(url), client, localFilePaths),
					),
				);
			},
		);
	...
	private async getStorageAccountName(): Promise<string> {
		let { userValue } = await userSettings.getConfigurationAsync(
	...
	private async createStorageAccountIfDoesNotExist(
		...
		try {
			const storageAccount =
				await client.storageResource.getStorageAccount(
					storageAccountName,
				);
			if (
				storageAccount.properties.provisioningState ===
	...
	private async createStorageAccount(
		...
		const action = async () =>
			isOnlyCheckStatus
				? null
				: await client.storageResource.createStorageAccount(
						storageAccountName,
						JSON.stringify(body),
					);

		const stateRequest = async () =>
			await client.storageResource.getStorageAccount(storageAccountName);
		const checkRequestStatusCallback = (
			storageAccount: IStorageAccountDto | null,
		) =>
			!storageAccount ||
			storageAccount.properties.provisioningState ===
				Constants.provisioningState.resolvingDns ||
			storageAccount.properties.provisioningState ===
				Constants.provisioningState.creating;

		return window.withProgress(
			{
				location: ProgressLocation.Window,
				title: Constants.statusBarMessages.createStorageAccount,
			},
			async () =>
	...
	private async createContainerIfDoesNotExist(
		...
		try {
			await BlobServiceClient.getContainer(
				storageAccountName,
				containerName,
				sas,
			);
			...
			if (
				!(error.message && error.message.includes("ContainerNotFound"))
			) {
				throw error;
			}
		}
		...
		await window.withProgress(
			{
				location: ProgressLocation.Window,
				title: Constants.statusBarMessages.createContainer,
			},
			async () =>
	...
	private async getStorageAccountSas(
		client: StorageAccountClient,
		storageAccountName: string,
	): Promise<string> {
		...
		return (
			await client.storageResource.getListAccountSas(
				storageAccountName,
				JSON.stringify(body),
			)
		).accountSasToken;
	...
	private async createBlob(
		...
		const url = await BlobServiceClient.createBlob(
			storageAccountName,
			containerName,
			blobName,
			sas,
	...
	private async deleteBlob(
		...
		const storageAccountName = urlWithAuth
			.match(/[a-z0-9]*(?=\.blob)/)!
			.toString();
	...
	private async getStorageAccountClient(
		subscriptionId: string,
		resourceGroup: string,
	): Promise<StorageAccountClient> {
