...
export class ConsortiumResourceExplorer extends AzureResourceExplorer {
	...
	public async selectProject(
		...
		if (pick instanceof ConsortiumItem) {
			Telemetry.sendEvent(
				"ConsortiumResourceExplorer.selectProject.selectAzureBlockchainProject",
			);
			return this.getAzureConsortium(azureClient, pick);
		} else {
			Telemetry.sendEvent(
				"ConsortiumResourceExplorer.selectProject.createAzureBlockchainProject",
			);
			return this.createAzureConsortium(azureClient, subscriptionItem);
		}
	}
	...
	public async getAccessKeys(
		transactionNetworkNode: AzureBlockchainNetworkNode,
	): Promise<string[]> {
		await this.waitForLogin();
		...
		const accessKeys =
			await azureClient.transactionNodeResource.getTransactionNodeAccessKeys(
				memberName,
				transactionNodeName,
			);
	...
	public async loadConsortiumItems(
		...
		const consortia: IAzureConsortiumDto[] =
			await azureClient.consortiumResource.getConsortiaList();
	...
	public async loadMemberItems(
		...
		const members: IAzureConsortiumMemberDto[] =
			await azureClient.memberResource.getMemberList(memberName);

		return members.filter(
			(member) =>
				member.status === Constants.consortiumMemberStatuses.ready,
		);
	}
	...
	public async loadTransactionNodeItems(
		...
		try {
			const transactionNodes =
				await azureClient.transactionNodeResource.getTransactionNodeList(
					memberName,
				);
			const networkNodes = transactionNodes.map((tn) => {
				return this.getTransactionNetworkNode(
					tn.name,
					subscriptionId,
					resourceGroup,
					memberName,
			...
			const defaultNode = this.getTransactionNetworkNode(
				memberName,
				subscriptionId,
				resourceGroup,
				memberName,
	...
	public async createAzureConsortium(
		...
		const region = await this.getOrSelectLocationItem(
			subscriptionItem,
			certainLocation,
		);
	...
	private async getAzureConsortium(
		...
		const { consortiumName, subscriptionId, resourceGroup, memberName } =
			consortiumItems;
		...
		const azureMembers = await Promise.all(
			memberItems.map(async (memberItem) => {
				const transactionNodeItems =
					await this.loadTransactionNodeItems(
						azureClient,
						memberItem.name,
					);
	...
	private getConsortiumName(
		...
		return this.getAzureBlockchainServiceName(
			...
			(name) =>
				AzureBlockchainServiceValidator.validateAzureBlockchainResourceName(
					name,
					azureClient.consortiumResource,
				),
		);
	}

	private getConsortiumMemberName(
		azureClient: AzureBlockchainServiceClient,
	): Promise<string> {
		return this.getAzureBlockchainServiceName(
	...
	private async getConsortiumItems(
		...
		const createConsortiumItem: QuickPickItem = {
			label: "$(plus) Create Consortium",
		};
	...
	private async createConsortium(
		...
		await azureClient.consortiumResource.createConsortium(
			memberName,
			bodyParams,
		);
		open(
