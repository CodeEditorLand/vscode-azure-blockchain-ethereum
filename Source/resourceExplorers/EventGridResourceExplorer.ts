...
import type { EventGridManagementClient } from "../ARMBlockchain/EventGridManagementClient";
...
export class EventGridResourceExplorer extends AzureResourceExplorer {
	public async loadEventGridItems(
		eventGridClient: EventGridManagementClient,
	): Promise<EventGridItem[]> {
		try {
			const eventGrids =
				await eventGridClient.eventGridResource.getEventGridList();

			return eventGrids
				.filter(
					(eg) =>
						eg.properties.provisioningState ===
