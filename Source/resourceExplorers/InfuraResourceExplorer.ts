...
import type {
	IInfuraEndpointDto,
	IInfuraProjectDto,
	IInfuraProjectQuickPick,
} from "../services/infuraService/InfuraDto";
...
export class InfuraResourceExplorer {
	...
	public async selectProject(
		...
		if (projectDestination instanceof InfuraProjectItem) {
			Telemetry.sendEvent(
				"InfuraResourceExplorer.selectProject.selectInfuraProject",
			);
			...
			Telemetry.sendEvent(
				"InfuraResourceExplorer.selectProject.creatInfuraProject",
			);
			return this.createInfuraProject(existingProjects);
		}
	}
	...
	public async getProjectsForQuickPick(): Promise<IInfuraProjectQuickPick[]> {
		...
		return allProjects.map((project: IInfuraProjectDto) => {
			return {
				...project,
				label: project.name,
				picked: !excludedProjects.some(
					(excluded) => excluded.id === project.id,
				),
			};
		});
	}

	private async getProjectDestinations(
		existingProjectIds: string[],
	): Promise<QuickPickItem[]> {
		const createInfuraProjectItem: QuickPickItem = {
			label: Constants.uiCommandStrings.createInfuraProject,
		};
		const infuraProjectItems =
			await this.loadInfuraProjectItems(existingProjectIds);

		return [createInfuraProjectItem, ...infuraProjectItems];
	}

	private async loadInfuraProjectItems(
		existingProjectIds: string[],
	): Promise<InfuraProjectItem[]> {
		const listOfProject: IInfuraProjectDto[] =
			await InfuraServiceClient.getAllowedProjects();

		return listOfProject
			.map(
				(project: IInfuraProjectDto) =>
					new InfuraProjectItem(
						project.name,
						project.id,
						project.endpoints,
					),
	...
	private async createInfuraProject(
		existingProjects: string[],
	): Promise<InfuraProject> {
		...
		const projectDetails = await InfuraServiceClient.getProjectDetails(
			newProject.id,
		);
	...
	private async getProjectName(existingProjects: string[]): Promise<string> {
		...
		return showInputBox({
			...
			validateInput: async (value: string) => {
				...
				if (
					!value.match(Constants.validationRegexps.infuraProjectname)
				) {
	...
	private async getInfuraProject(
		...
		for (const [key, value] of Object.entries(endpoints)) {
			infuraNetworkNodes.push(
				new InfuraNetworkNode(
					key,
					value.https,
					Constants.infuraEndpointsIds[key],
				),
			);
		}
		...
		infuraProject.setChildren(
			infuraNetworkNodes.sort(
				(first, second) =>
					(first.networkId as number) - (second.networkId as number),
			),
		);
	...
	private async waitForLogin(): Promise<void> {
		...
		if (shouldSignIn) {
			...
			const error = new Error(
				Constants.errorMessageStrings.InfuraUnauthorized,
			);
