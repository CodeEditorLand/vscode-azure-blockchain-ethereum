...
import {
	ContractDB,
	type ContractInstanceWithMetadata,
	ContractService,
	TreeManager,
} from "../services";
...
import type { StorageAccountResourceExplorer } from "./StorageAccountResourceExplorer";
...
export class BlockchainDataManagerResourceExplorer extends AzureResourceExplorer {
	public async selectProject(
		...
		createConsortiumCallback: (
			consortium: AzureBlockchainProject,
		) => Promise<void>,
	): Promise<BlockchainDataManagerProject> {
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.selectProject",
		);
		await this.waitForLogin();

		const subscriptionItem = await this.getOrSelectSubscriptionItem();
		const resourceGroupItem =
			await this.getOrCreateResourceGroupItem(subscriptionItem);
		...
		const pick = await showQuickPick(
			[
				{
					label: Constants.uiCommandStrings
						.createBlockchainDataManagerProject,
				},
				...(await this.getBlockchainDataManagerInstanceItems(
					azureClient,
					existingProjects,
				)),
			],
			{
				placeHolder:
					Constants.placeholders.selectBlockchainDataManagerInstance,
				ignoreFocusOut: true,
			},
		);
		...
		if (pick instanceof BlockchainDataManagerInstanceItem) {
			Telemetry.sendEvent(
				"BlockchainDataManagerResourceExplorer.selectProject.selectBlockchainDataManagerProject",
			);
			return this.getBlockchainDataManagerInstance(pick, azureClient);
		} else {
			Telemetry.sendEvent(
				"BlockchainDataManagerResourceExplorer.selectProject.createBlockchainDataManagerProject",
			);
	...
	public async createProject(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.createProject",
		);
		await this.waitForLogin();
		...
		if (!azureClient || !subscriptionItem) {
			...
			resourceGroupItem =
				await this.getOrCreateResourceGroupItem(subscriptionItem);
			azureClient = await this.getAzureClient(
				subscriptionItem,
				resourceGroupItem,
			);
		}

		const eventGridClient = await this.getEventGridClient(
			subscriptionItem,
			resourceGroupItem as ResourceGroupItem,
		);
		const selectedConsortium = await this.getSelectedConsortium(
		...
		if (!(selectedConsortium instanceof ConsortiumItem)) {
			const child =
				await consortiumResourceExplorer.createAzureConsortium(
					azureClient,
					subscriptionItem,
			...
			Telemetry.sendEvent(
				"BlockchainDataManagerResourceExplorer.createProject.createAzureBlockchainProject",
			);
		...
		const bdmItems =
			await azureClient.bdmResource.getBlockchainDataManagerList();
		...
		const selectedEventGrid = await this.getSelectedEventGrid(
			eventGridClient,
			selectedConsortium.location,
		);
		...
		return this.createBlockchainDataManagerInstance(
			...
			selectedConsortium.location,
			selectedMember.label,
			Constants.getTransactionNodeName(
				selectedMember.label,
				selectedTransactionNode.transactionNodeName,
			),
	...
	public async deleteBDMApplication(
		...
		await azureClient.bdmResource.deleteBlockchainDataManagerApplication(
			bdmLabel,
			label,
		);
		...
		await storageAccountResourceExplorer.deleteBlobs(
			fileUrls,
			subscriptionId,
			resourceGroup,
			localFilePaths,
	...
	public async createNewBDMApplication(
		...
		const solidityContractsFolderPath =
			await ContractService.getSolidityContractsFolderPath();
		const contracts = this.getFiles(
			solidityContractsFolderPath,
			Constants.contractExtension.sol,
		);
		...
		const instance = contractInstances[
		...
		const [byteCode, applicationName] = await Promise.all([
			ContractService.getDeployedBytecodeByAddress(
				instance.provider.host,
				instance!.address,
			),
		...
		window.showInformationMessage(
			Constants.informationMessage.bdm.bdmApplicationNotReady,
		);

		const [abiUrl, bytecodeUrl] = await this.getBlobUrls(
			applicationName,
		...
		return this.createBDMApplication(
			applicationName,
			selectedBDM,
			bytecodeUrl,
			abiUrl,
		...
		await window.withProgress(
			{
				...
				title: Constants.statusBarMessages.createBDMApplication,
			},
			async () => {
				const application =
					await this.createBlockchainDataManagerApplication(
						applicationName,
						selectedBDM,
						bytecodeUrl,
						abiUrl,
	...
	private async compiledAndDeployContracts(): Promise<void> {
		const answer = await window.showErrorMessage(
			Constants.informationMessage.bdm
	...
	private async createBlockchainDataManagerApplication(
		...
		const azureClient = await this.getAzureClient(
			subscriptionItem,
			new ResourceGroupItem(bdm.resourceGroup),
		);

		const body: ICreateBlockchainDataManagerApplicationDto = {
			properties: {
				artifactType:
					Constants.bdmApplicationRequestParameters.artifactType,
				content: {
					abiFileUrl: abiUrl,
					bytecodeFileUrl: bytecodeUrl,
					queryTargetTypes:
						Constants.bdmApplicationRequestParameters
							.queryTargetTypes,
				},
			},
		};

		return azureClient.bdmResource.createBlockchainDataManagerApplication(
			bdm.label,
			applicationName,
			body,
		);
	...
	private async getBDMApplicationName(
		bdm: BlockchainDataManagerProject,
		contractName: string,
	): Promise<string> {
		const subscriptionItem = await this.getSubscriptionItem(
		...
		const applicationList =
			await azureClient.bdmResource.getBlockchainDataManagerApplicationList(
				bdm.label,
			);
		const existingNames = applicationList.map(
		...
		return showInputBox({
			...
			validateInput: async (name) => {
				return AzureBlockchainServiceValidator.validateBDMApplicationName(
					name,
					existingNames,
				);
			},
	...
	private async getBlockchainDataManagerInstanceItems(
		...
		const bdmItems = await this.loadBlockchainDataManagerInstanceItems(
			azureClient,
			excludedItems,
		);
	...
	private async getBlockchainDataManagerInstance(
		...
		const bdmProject = new BlockchainDataManagerProject(
			bdmName,
			subscriptionId,
			resourceGroup,
		);
		const bdmApplications = await this.getBlockchainDataManagerApplications(
			bdmInstanceItem,
			azureClient,
		);

		const bdmInputList =
			await azureClient.bdmResource.getBlockchainDataManagerInputList(
				bdmName,
			);
		const bdmInputs = await this.getBlockchainDataManagerInputs(
			bdmInputList,
			subscriptionId,
			resourceGroup,
		);
		...
		const bdmOutputList =
			await azureClient.bdmResource.getBlockchainDataManagerOutputList(
				bdmName,
			);
		const bdmOutputs = await this.getBlockchainDataManagerOutputs(
			bdmOutputList,
			subscriptionId,
			resourceGroup,
		);
	...
	private async getBlockchainDataManagerApplications(
		...
		const bdmApplicationList =
			await azureClient.bdmResource.getBlockchainDataManagerApplicationList(
				bdmName,
			);

		return bdmApplicationList.map((application) => {
			const { abiFileUrl, bytecodeFileUrl } =
				application.properties.content;
			const indexRemovingRoute = application.id.lastIndexOf("/artifacts");
			const url = `${
				Constants.azureResourceExplorer.portalBasUri
			}/resource${application.id.slice(
				0,
				indexRemovingRoute,
	...
	private async getBlockchainDataManagerInputs(
		...
		const bdmInputs = bdmInputList.map((input) => {
			return new BlockchainDataManagerNetworkNode(
				...
				this.getTransactionNodeExternalUrl(
					input.properties.dataSource.resourceId,
				),
			);
		});
	...
	private async createBlockchainDataManagerInstance(
		...
		const bodyParamsForCreateBDMInput: ICreateBlockchainDataManagerInputDto =
			{
				properties: {
					dataSource: {
						...
						resourceId: this.getTransactionNodeUrl(
							subscriptionId,
							resourceGroup,
							memberName,
							transactionNodeName,
		...
		const connectionName =
			await this.getBlockchainDataManagerConnectionName();
		...
		return await window.withProgress(
			...
			async () => {
				const createdBDM =
					await bdmResource.createBlockchainDataManager(
						bdmName,
						bodyParamsForCreateBDM,
					);
				const createdInput =
					await bdmResource.createBlockchainDataManagerInput(
						bdmName,
						transactionNodeName,
						bodyParamsForCreateBDMInput,
					);
				const createdOutput =
					await bdmResource.createBlockchainDataManagerOutput(
						bdmName,
						connectionName,
						bodyParamsForCreateBDMOutput,
					);
				await this.startBlockchainDataManager(
					bdmResource,
					createdBDM.id,
					bdmName,
				);
		...
		return showInputBox({
			...
			validateInput: (name: string) => {
				if (
					!name ||
					!name.match(
						Constants.validationRegexps.forbiddenChars
							.outboundConnectionName,
					)
				) {
					return Constants.validationMessages.forbiddenChars
						.outboundConnectionName;
				}
	...
	private getBlockchainDataManagerName(
		bdmItems: IAzureBlockchainDataManagerDto[],
	): Promise<string> {
		return showInputBox({
			ignoreFocusOut: true,
			...
			validateInput: (name: string) => {
				if (
					!name ||
					!name.match(
						Constants.validationRegexps.specialChars.bdmName,
					)
				) {
	...
	private getTransactionNodeName(
		...
		return showInputBox({
			...
			validateInput: (name: string) => {
				if (
					!name ||
					!name.match(
						Constants.validationRegexps.specialChars
							.transactionNodeName,
					)
				) {
				...
				if (transactionNodeItems.some((tn) => tn.label === name)) {
					return Constants.validationMessages
						.transactionNodeNameAlreadyExists;
				}
		...
		return showInputBox({
			...
			validateInput: (name: string) => {
				if (
					!name ||
					!name.match(
						Constants.validationRegexps.specialChars.eventGridName,
					)
				) {
	...
	private async getSelectedConsortium(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.getSelectedConsortium.selectConsortium",
		);

		const consortiumItems =
			await consortiumResourceExplorer.loadConsortiumItems(azureClient);
		const filteredConsortia = consortiumItems.filter((con) =>
			Constants.availableBlockchainDataManagerLocations.includes(
				con.location,
			),
		);
	...
	private async getSelectedMember(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.getSelectedMember.selectMember",
		);

		const members = await consortiumResourceExplorer.loadMemberItems(
			azureClient,
			memberName,
		);
		const memberItems: QuickPickItem[] = members.map((member) => ({
			label: member.name,
		}));

		return showQuickPick(memberItems, {
			placeHolder: Constants.placeholders.selectMember,
			ignoreFocusOut: true,
		});
	}
	...
	private async getSelectedTransactionNode(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.getSelectedTransactionNode.selectTransactionNode",
		);
		...
		await window.withProgress(
			...
			async () => {
				...
				await Promise.all(
					bdmItems.map((bdm) =>
						bdmResource.getBlockchainDataManagerInputList(bdm.name),
					),
				).then((result) => {
				...
				const transactionNodes =
					await transactionNodeResource.getTransactionNodeList(
						selectedMemberName,
					);
				const defaultTransactionNode =
					await transactionNodeResource.getTransactionNode(
						selectedMemberName,
				...
				transactionNodeItems = [
					defaultTransactionNode,
					...transactionNodes,
				].map(
					(tn) =>
						new TransactionNodeItem(
							tn.name,
							tn.id,
							tn.properties.provisioningState,
						),
				);

				filteredTransactionNode = transactionNodeItems.filter(
					(tn) =>
						tn.provisioningState ===
							Constants.provisioningState.succeeded &&
						!bdmInputList.some(
							(input) =>
								input.properties.dataSource.resourceId ===
								tn.url,
		...
		const selectedTransactionNode = await showQuickPick(
			[
				{ label: Constants.uiCommandStrings.createTransactionNode },
				...filteredTransactionNode,
			],
			{
				placeHolder: Constants.placeholders.selectTransactionNode,
				ignoreFocusOut: true,
			},
		);
		...
		await this.createdTransactionNode(
			selectedMemberName,
			location,
			azureClient,
			transactionNodeItems,
	...
	private async getSelectedEventGrid(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.getSelectedEventGrid.selectEventGrid",
		);

		const eventGridResourceExplore = new EventGridResourceExplorer();
		const eventGridList =
			await eventGridResourceExplore.loadEventGridItems(eventGridClient);

		const selectedEventGrid = await showQuickPick(
			[
				{ label: Constants.uiCommandStrings.createEventGrid },
				...eventGridList,
			],
			{
				placeHolder: Constants.placeholders.selectEventGrid,
				ignoreFocusOut: true,
			},
		);
		...
		await this.getCreatedEventGrid(
			eventGridClient,
			location,
			eventGridList,
		);
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.getSelectedEventGrid.cancellationCreatingEventGrid",
		);
		throw new CancellationEvent();
	}
	...
	private async getCreatedEventGrid(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.getCreatedEventGrid.createEventGrid",
		);
		...
		const eventGrid =
			await eventGridClient.eventGridResource.createEventGrid(
				eventGridName,
				JSON.stringify(bodyParamsForCreateEventGrid),
			);

		showNotification({
			message: Constants.informationMessage.provisioningResource(
				Constants.azureBlockchainResourceName.eventGrid,
			),
			type: NotificationOptions.info,
		});
		open(
			`${Constants.azureResourceExplorer.portalBasUri}/resource/${eventGrid.id}`,
		);
	}
	...
	private async createdTransactionNode(
		...
		Telemetry.sendEvent(
			"BlockchainDataManagerResourceExplorer.createdTransactionNode.createTransactionNode",
		);

		const transactionNodeName =
			await this.getTransactionNodeName(transactionNodeItems);
		const transactionNodePassword = await showInputBox({
			ignoreFocusOut: true,
			password: true,
			prompt: Constants.paletteLabels.enterTransactionNodePassword,
			validateInput:
				AzureBlockchainServiceValidator.validateAccessPassword,
		});
		...
		const createdTransactionNode =
			await azureClient.transactionNodeResource.createTransactionNode(
				memberName,
				transactionNodeName,
		...
		showNotification({
			message: Constants.informationMessage.provisioningResource(
				Constants.azureBlockchainResourceName.transactionNode,
			),
			type: NotificationOptions.info,
		});
	...
	private getTransactionNodeExternalUrl(url: string): string {
		...
		const idxOfMemberAndTransactionNode =
			url.lastIndexOf(firstMark) + firstMark.length;
		const relativeInternalPath = url
			.substring(0, idxOfMemberAndTransactionNode)
			.replace(/\//g, "%2F");
		let memberAndTransactionNode = url
			.substring(idxOfMemberAndTransactionNode)
			.replace("transactionNodes", nodeName);

		if (
			memberAndTransactionNode.includes(Constants.defaultInputNameInBdm)
		) {
			const memberName = memberAndTransactionNode.substring(
				0,
				memberAndTransactionNode.indexOf("/" + nodeName),
			);
			memberAndTransactionNode = memberAndTransactionNode.replace(
				Constants.defaultInputNameInBdm,
				memberName,
			);
		}
	...
	private async getAzureClient(
		...
		return new AzureBlockchainServiceClient(
			...
			{
				acceptLanguage:
					Constants.azureResourceExplorer.requestAcceptLanguage,
				filters: [],
