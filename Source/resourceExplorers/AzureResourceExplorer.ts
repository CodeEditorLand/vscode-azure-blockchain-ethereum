...
export class AzureResourceExplorer {
	...
	public async getOrCreateResourceGroupItem(
		...
		if (pick instanceof ResourceGroupItem) {
			Telemetry.sendEvent(
				"AzureResourceExplorer.getOrCreateResourceGroupItem.itemIsResourceGroupItem",
			);
			return pick;
		} else {
			Telemetry.sendEvent(
				"AzureResourceExplorer.getOrCreateResourceGroupItem.createResourceGroupItemSelected",
			);
	...
	public async getOrSelectLocationItem(
		...
		const filteredLocations = certainLocation?.length
			? locationItems.filter((location) =>
					certainLocation.includes(location.description),
				)
			: locationItems;
		...
		if (!result) {
			...
			if (!result) {
				const error = new Error(
					Constants.errorMessageStrings.WaitForLogin,
				);
	...
	public async getSubscriptionItem(
	...
	private async getResourceGroupItems(
		...
		const cachedLocationItems =
			await this.getCachedLocationItems(subscriptionItem);
		...
		const resourceItems = resourceGroups.map((resourceGroup) => {
			const location = locationItems.find(
				(locationItem) =>
					locationItem.description === resourceGroup.location,
			);
	...
	private async getLocationItems(
		...
		return providerLocationItems.length !== 0
	...
	private async createResourceGroup(
		...
		const resourceGroupName = await showInputBox({
			...
			validateInput: (name) =>
				AzureBlockchainServiceValidator.validateResourceGroupName(
					name,
					resourceGroups,
				),
		});
		...
		return window.withProgress(
			...
			async () => {
				if (subscriptionItem.subscriptionId === undefined) {
					...
					Telemetry.sendEvent(
						"AzureResourceExplorer.createResourceGroup.withProgress.subscriptionIdIsDefined",
					);
					const resourceManagementClient =
						await this.getResourceClient(subscriptionItem);
					const resourceGroup =
	...
	private async getCachedLocationItems(
		subscriptionItem: SubscriptionItem,
	): Promise<ICachedLocationItems> {
		...
		const subscriptionClient =
			await this.getSubscriptionClient(subscriptionItem);
		const resourceManagementClient =
			await this.getResourceClient(subscriptionItem);
		const locations = await subscriptionClient.subscriptions.listLocations(
			subscriptionItem.subscriptionId,
		);
		const blockchain = await resourceManagementClient.providers.get(
			Constants.azureResourceExplorer.providerName,
		);
		const locationItems = locations.map(
			(location) => new LocationItem(location.displayName, location.name),
		);
		...
		const blockchainMember =
			...
			blockchain.resourceTypes.find((resourceType) => {
				return (
					...
					Constants.azureResourceExplorer.resourceType
				);
			});
		...
		if (blockchainMember && blockchainMember.locations) {
			providerLocationItems.push(
				...locationItems
					.filter((item) =>
						blockchainMember.locations!.includes(item.label),
					)
