...
export class LocalResourceExplorer {
	...
	private async getOrCreateLocalProject(
		...
		const localProjectName =
			await this.getLocalProjectName(existingProjects);
		const localProjectPort = await this.getLocalProjectPort(
			existingPorts,
			portStatus,
			validateMessage,
		);
	...
	private async getLocalProjectName(
		existingProjects: string[],
	): Promise<string> {
		return showInputBox({
			ignoreFocusOut: true,
			...
			validateInput: (value: string) => {
				const validationError =
					DialogResultValidator.validateLocalNetworkName(value);
				if (validationError) {
			...
			value: existingProjects.includes(Constants.localhostName)
				? ""
				: Constants.localhostName,
		});
	}
	...
	private async getLocalProjectPort(
		...
		const port = await showInputBox({
			...
			validateInput: async (value: string) => {
				...
				if (
					existingPorts.some((existPort) => existPort + "" === value)
				) {
					return Constants.validationMessages.projectAlreadyExists;
				}

				if (
					(await GanacheService.getPortStatus(value)) !== portStatus
				) {
			...
			value: existingPorts.includes(Constants.defaultLocalhostPort)
				? ""
