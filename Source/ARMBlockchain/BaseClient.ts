...
export class BaseClient extends AzureServiceClient {
	constructor(
		...
		if (!credentials) {
			const error = new Error(
				Constants.errorMessageStrings.VariableShouldBeDefined(
					"Credentials",
				),
			);
			...
			(
				...
				if (err) {
					Telemetry.sendException(
						new Error(
							"BaseClient.sendRequestToAzure.pipeline.error",
						),
					);
					...
					Telemetry.sendException(
						new Error(
							"BaseClient.sendRequestToAzure.pipeline.statusCodeNotSuccess",
						),
					);
					...
					try {
						const parsedResult = responseBody
							? JSON.parse(responseBody)
							: null;
						return callback(null, parsedResult);
					} catch (error) {
						Telemetry.sendException(
							new Error("Unexpected token in JSON at position 1"),
						);
						return callback(
							new Error(
								`Error ${error.message} occurred in deserialize the responseBody`,
							),
						);
					}
				}
	...
	public getHttpRequest(
		...
		httpRequest.headers["Content-Type"] =
			Constants.azureResourceExplorer.contentType;
