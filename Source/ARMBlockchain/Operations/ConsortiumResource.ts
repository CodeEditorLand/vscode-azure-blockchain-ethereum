...
import type { IAzureConsortiumDto, IAzureMemberDto } from "..";
import type { AzureBlockchainServiceClient } from "../AzureBlockchainServiceClient";
import {
	ConsortiumMapper,
	type ICreateQuorumMember,
} from "../Mapper/ConsortiumMapper";
...
export class ConsortiumResource {
	...
	public createConsortium(
		...
		return new Promise((resolve, reject) => {
			return this.client.createConsortium(
				memberName,
				JSON.stringify(body),
	...
	public async checkExistence(name: string): Promise<{
		...
		return this.client.checkExistence(
			name,
			"Microsoft.Blockchain/consortiums",
		);
	}

	public async getConsortiaList(): Promise<IAzureConsortiumDto[]> {
		return new Promise((resolve, reject) => {
			return this.client.getConsortia(
				(error: Error | null, result?: any) => {
					if (error) {
						reject(error);
					} else {
