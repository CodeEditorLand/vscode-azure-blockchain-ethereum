...
import type { AzureBlockchainServiceClient } from "../AzureBlockchainServiceClient";

export class BlockchainDataManagerResource {
	constructor(public readonly client: AzureBlockchainServiceClient) {}

	public async getBlockchainDataManagerList(): Promise<
		IAzureBlockchainDataManagerDto[]
	> {
		return new Promise((resolve, reject) => {
			return this.client.getBlockchainDataManagers(
	...
	public async getBlockchainDataManagerInputList(
		bdmName: string,
	...
	public async getBlockchainDataManagerOutputList(
		bdmName: string,
	): Promise<IAzureBlockchainDataManagerOutputDto[]> {
		return new Promise((resolve, reject) => {
			return this.client.getBlockchainDataManagerOutputs(
				bdmName,
				(error: Error | null, result?: any) => {
					if (error) {
						reject(error);
	...
	public startBlockchainDataManager(bdmName: string): Promise<void> {
		return new Promise((resolve, reject) => {
			return this.client.startBlockchainDataManager(
				bdmName,
				(error: Error | null, result?: any) => {
					if (error) {
						reject(error);
	...
	private removeBlockchainDataManager(bdmName: string): Promise<void> {
		return new Promise((resolve, reject) => {
			return this.client.removeBlockchainDataManager(
				bdmName,
				(error: Error | null, result?: any) => {
					if (error) {
						reject(error);
