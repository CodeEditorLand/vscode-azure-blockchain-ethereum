...
export class AzureBlockchainServiceClient extends BaseClient {
	...
	public createConsortium(
		...
		return this.sendRequestToAzure(httpRequest, (error) => {
			if (error) {
				Telemetry.sendEvent(
					"AzureBlockchainServiceClient.createConsortium.createdFailed",
				);
				callback(error);
			} else {
				Telemetry.sendEvent(
					"AzureBlockchainServiceClient.createConsortium.createdSuccessfully",
				);
				callback(null);
			}
		});
	...
	public startBlockchainDataManager(
		...
		return this.sendRequestToAzure(httpRequest, (error) => {
			if (error) {
				Telemetry.sendEvent(
					"AzureBlockchainServiceClient.startBlockchainDataManager.createdFailed",
				);
				callback(error);
			} else {
				Telemetry.sendEvent(
					"AzureBlockchainServiceClient.startBlockchainDataManager.runSuccessfully",
				);
				callback(null);
			}
		});
	...
	public createBlockchainDataManagerOutput(
		...
		const url = this.getUrl(
			`watchers/${bdmName}/outputs/${connectionName}`,
			preview20190601,
			true,
			false,
		);
	...
	public createBlockchainDataManagerApplication(
		...
		const url = this.getUrl(
			`watchers/${bdmName}/artifacts/${applicationName}`,
			preview20190601,
			true,
			false,
		);
	...
	public getBlockchainDataManagers(
		callback: (error: Error | null, result?: any) => void,
	): Promise<void> {
	...
	public getTransactionNode(
		...
		const url = this.getUrl(
			`${memberName}/transactionNodes/${transactionNodeName}`,
			preview20180601,
			true,
			true,
		);
	...
	public checkExistence(
		...
		const url = this.getUrl(
			`locations/${this.location}/checkNameAvailability`,
			preview20180601,
		);
	...
	private getUrl(
		...
		useBlockchainMembers = false,
	): string {
		const resourceGroup = useResourceGroup
			? `resourceGroups/${this.resourceGroup}/`
			: "";
		const blockchainMember = useBlockchainMembers
			? "blockchainMembers/"
			: "";
