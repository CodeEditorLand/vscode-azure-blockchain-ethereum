...
export class AzureBlockchainServiceClient extends AzureServiceClient {
	...
	public async createConsortium(
		...
		await this.pipeline(httpRequest, (err: ServiceError) => {
			...
			env.openExternal(
				Uri.parse(
					`${Constants.azurePortalBasUri}/resource/${urlDetailsOfConsortium}`,
				),
			);
		});
	}
	...
	public getMembers(
		...
		const url =
			`${this.baseUri}/subscriptions/${this.subscriptionId}/resourceGroups/${this.resourceGroup}` +
		...
		return await this.pipeline(
			...
			(
				...
				try {
					...
					return callback(
						new Error(
							`Error ${error} occurred in deserialize the responseBody - ${responseBody}`,
						),
					);
				}
			},
