...
import { type Extension, commands, extensions } from "vscode";
import { Constants } from "../../Constants";
import type { IExtensionAdapter } from "./IExtensionAdapter";
...
export class OpenZeppelinExtensionAdapter implements IExtensionAdapter {
	...
	constructor() {
		this.extensionInfo =
			Constants.externalExtensions[Constants.coreSdk.openZeppelin];
	...
	public async validateExtension(): Promise<void> {
		if (!this.extension) {
			throw new Error(
				Constants.errorMessageStrings.ExtensionNotInstalled(
	...
	public async build(): Promise<void> {
		return commands.executeCommand(
			this.extensionInfo.commands.buildContracts,
		);
	}

	public async deploy(): Promise<void> {
		return commands.executeCommand(
			this.extensionInfo.commands.deployContracts,
		);
	}
}
