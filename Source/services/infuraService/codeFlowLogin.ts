...
export async function signIn() {
	...
	try {
		...
		const authorizationUrl = new url.URL(
			Constants.infuraAuthUrls.authURL,
			Constants.infuraAuthUrls.baseURL,
		);
...
export async function signOut(token: string): Promise<void> {
	const queryParams = queryString({
		grant_type: Constants.infuraRequestGrantType.authorizationCode,
		token,
	});
	const options = {
		body: queryParams,
	...
	const requestUrl = new url.URL(
		Constants.infuraAuthUrls.revoke,
		Constants.infuraAuthUrls.baseURL,
	);
...
export async function refreshToken(token: string): Promise<IToken> {
	return await tokenRequest({
		grant_type: Constants.infuraRequestGrantType.refreshToken,
		refresh_token: token,
	});
}
...
async function tokenRequest(params: any): Promise<IToken> {
	...
	const requestUrl = new url.URL(
		Constants.infuraAuthUrls.tokenURL,
		Constants.infuraAuthUrls.baseURL,
	);
	...
	if (Number.isInteger(result.expires_in)) {
		expirationDate.setSeconds(
			expirationDate.getSeconds() + result.expires_in,
		);
	}
...
function createServer() {
	...
	const server = http.createServer((req, res) => {
		const reqUrl = new url.URL(
			req.url!,
			`${Constants.networkProtocols.http}${Constants.localhost}`,
		);
		switch (reqUrl.pathname) {
			...
			case "/":
				sendFile(
					res,
					Constants.infuraFileResponse.path,
			...
			case "/main.css":
				sendFile(
					res,
					Constants.infuraFileResponse.css,
