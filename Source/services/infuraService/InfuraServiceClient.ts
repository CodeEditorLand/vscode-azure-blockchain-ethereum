...
import { type IToken, refreshToken, signIn, signOut } from "./codeFlowLogin";
...
class InfuraClient {
	...
	private readonly showProjectsFromInfuraCommand =
	...
	public async isSignedIn(): Promise<boolean> {
		...
		if (infuraCache && infuraCache.tokens) {
			const isTokenExpired =
				infuraCache.tokens.accessTokenExpirationDate < new Date();
			if (isTokenExpired) {
	...
	public async signOut() {
		...
		if (
			infuraCache &&
			infuraCache.tokens &&
			infuraCache.tokens.accessToken
		) {
	...
	public async getUserData(): Promise<IInfuraUserDto> {
		const url = new URL(
			Constants.infuraAPIUrls.userMe,
	...
	public getExcludedProjects(): IInfuraProjectDto[] {
		if (this.globalState) {
			return (
				this.globalState.get<IInfuraProjectDto[]>(
					Constants.globalStateKeys.infuraExcludedProjectsListKey,
				) || []
			);
		} else {
			return [];
	...
	public async createProject(
		...
		const url = new URL(
			Constants.infuraAPIUrls.projects,
			Constants.infuraAPIUrls.rootURL,
		).toString();
		const params = {
			body: project,
	...
	private getInfuraCache(): IInfuraCache | undefined {
		return this.globalState
			? this.globalState.get<IInfuraCache>(
					Constants.globalStateKeys.infuraCredentialsCacheKey,
				)
			: undefined;
	}

	private async updateInfuraCache(
		newInfuraCache?: IInfuraCache,
	): Promise<void> {
		if (this.globalState) {
			await this.globalState.update(
				Constants.globalStateKeys.infuraCredentialsCacheKey,
	...
	private async signInSilently(): Promise<boolean> {
		...
		if (
			infuraCache &&
			infuraCache.tokens &&
			infuraCache.tokens.refreshToken
		) {
			try {
				this.eventEmitter.fire(Constants.infuraSigningIn);
	...
	private addTokenToParams(params: any): any {
		...
		if (infuraCache) {
			return {
				...params,
				auth: { bearer: infuraCache.tokens.accessToken },
			};
		}
