...
export namespace GanacheService {
	...
	export async function getPortStatus(
		...
		if (!isNaN(await shell.findPid(port))) {
			if (await isGanacheServer(port)) {
				Telemetry.sendEvent(
					"GanacheService.isGanacheServerRunning.isGanacheServer",
					{ port: "" + port },
				);
				return PortStatus.GANACHE;
			} else {
				Telemetry.sendEvent(
					"GanacheService.isGanacheServerRunning.portIsBusy",
					{ port: "" + port },
				);
		...
		Telemetry.sendEvent(
			"GanacheService.isGanacheServerRunning.portIsFree",
			{ port: "" + port },
		);
	...
	export async function startGanacheServer(
		...
		if (UrlValidator.validatePort(port)) {
			Telemetry.sendException(
				new Error(Constants.ganacheCommandStrings.invalidGanachePort),
			);
			throw new Error(
				`${Constants.ganacheCommandStrings.invalidGanachePort}: ${port}.`,
			);
		}
		...
		if (portStatus === PortStatus.NOT_GANACHE) {
			Telemetry.sendException(
				new Error(Constants.ganacheCommandStrings.ganachePortIsBusy),
			);
			throw new Error(
				`${Constants.ganacheCommandStrings.ganachePortIsBusy}: ${port}.`,
			);
		}
		...
		Telemetry.sendEvent(
			"GanacheServiceClient.waitGanacheStarted.serverStarted",
		);
		return ganacheProcesses[port];
	}

	export async function stopGanacheServer(
		port: number | string,
		killOutOfBand = true,
	): Promise<void> {
	...
	export function getPortFromUrl(url: string): string {
		...
		return result
			? result[0].slice(1)
			: Constants.defaultLocalhostPort.toString();
	}

	export async function dispose(): Promise<void> {
		const shouldBeFree = Object.values(ganacheProcesses).map(
			(ganacheProcess) => stopGanacheProcess(ganacheProcess, false),
		);
		return Promise.all(shouldBeFree).then(() => undefined);
	}

	async function spawnGanacheServer(
		port: number | string,
	): Promise<IGanacheProcess> {
		const process = spawnProcess(undefined, "npx", [
			RequiredApps.ganache,
			`-p ${port}`,
		]);
		const output = window.createOutputChannel(
			`${Constants.outputChannel.ganacheCommands}:${port}`,
		);
	...
	async function stopGanacheProcess(
		ganacheProcess: IGanacheProcess,
		killOutOfBand: boolean,
	): Promise<void> {
		if (!ganacheProcess) {
			return;
		}
