...
export namespace OpenZeppelinProjectJsonService {
	export function getProjectJson(): IProjectMetadata {
		...
		return (
			fs.readJSONSync(projectJsonPath, {
				throws: false,
				encoding: "utf8",
			}) ||
	...
	export async function getReferencesToLibrariesAsync(
		contract: IOZAsset,
	): Promise<IOZAsset[]> {
		const result: IOZAsset[] = [];
		const projectMetadata = await getProjectJson();
		const projectAssets = projectMetadata.openZeppelin.assets;
		const contractDependencies = projectAssets.find(
			(asset: IOZAsset) => asset.id === contract.id,
		);

		if (
			!contractDependencies ||
			contractDependencies.dependencies === undefined
		) {
			return result;
		}

		for (const dependencyId of contractDependencies.dependencies) {
			const dependency = projectAssets.find(
				(asset: IOZAsset) => asset.id === dependencyId,
			)!;
			...
			if (dependency.type === OZAssetType.library) {
				...
				(await getReferencesToLibrariesAsync(dependency)).forEach(
					(refs: IOZAsset) => {
	...
	export async function addAssetsToProjectJsonAsync(
		...
		projectMetadata.openZeppelin.assets.forEach((storedAsset) => {
			if (
				!downloadedAssets.some(
					(downloaded) => downloaded.id === storedAsset.id,
				)
			) {
	...
	export async function addVersionToProjectJsonAsync(
		...
		const projectMetadata =
			updatingProjectMetadata || (await getProjectJson());
	...
	export async function addCategoryToProjectJsonAsync(
		...
		if (
			...
			projectMetadata.openZeppelin.version === Constants.firstOZVersion
		) {
			return projectMetadata;
		}

		projectMetadata.openZeppelin.categories =
			projectMetadata.openZeppelin.categories || [];

		if (
			projectMetadata.openZeppelin.categories.indexOf(category.id) === -1
		) {
	...
	export async function storeProjectJsonAsync(
		content: IProjectMetadata,
		destinationPath?: string,
	): Promise<void> {
		const projectJsonPath = getProjectJsonPath();

		return fs.writeFile(
			destinationPath || projectJsonPath,
