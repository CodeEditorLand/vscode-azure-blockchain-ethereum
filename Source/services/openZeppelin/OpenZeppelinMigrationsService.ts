...
import { type IOZAsset, OZAssetType } from "./models";
...
export namespace OpenZeppelinMigrationsService {
	export async function generateMigrations(items: IOZAsset[]): Promise<void> {
		const declarationSection =
			generateDeclarationSection(items).concat("\n");
		const contractDeployerSection = generateContractDeployerSection(items);
		const librariesDeployerSection =
			generateLibrariesDeployerSection(items);
		const librariesLinkingSection = (
			await generateLinkingSection(items)
		).join("\n");
...
function generateDeclarationSection(items: IOZAsset[]): string {
	return items
		.filter(
			(asset: IOZAsset) =>
				OpenZeppelinService.assetHasContracts(asset) ||
				asset.type === OZAssetType.library,
		)
		.reduce((declarations: string[], asset: IOZAsset) => {
			const contractNames =
				OpenZeppelinService.getContractsNamesFromAsset(asset);
			...
			return deployerSections.concat(
				contractNames.map((contract) => {
					const contractParameters =
						OpenZeppelinService.getContractParametersFromAsset(
							asset,
							contract,
						);
...
function generateLibrariesDeployerSection(items: IOZAsset[]): string {
	return items
		...
		.reduce((deployerSections: string[], asset: IOZAsset) => {
			const libraryName =
				OpenZeppelinService.getContractNameByAssetName(asset);
...
async function generateLinkingSection(items: IOZAsset[]): Promise<string[]> {
	...
	const contracts = items.filter((asset) =>
		OpenZeppelinService.assetHasContracts(asset),
	);
	...
	for (const contract of contracts) {
		librariesLinkingSection.push(
			...contractToLibraryLinkingSection(
				contract,
				await OpenZeppelinProjectJsonService.getReferencesToLibrariesAsync(
					contract,
				),
			),
		);
...
function contractToLibraryLinkingSection(
	baseAsset: IOZAsset,
	libraries: IOZAsset[],
): string[] {
	const contractsNames =
		OpenZeppelinService.getContractsNamesFromAsset(baseAsset);
	const librariesNames = libraries.map(
		OpenZeppelinService.getContractNameByAssetName,
	);

	return contractsNames.reduce(
		(deployerLinks: string[], contractName: string) => {
			return deployerLinks.concat(
				librariesNames.map(
					(libraryName) =>
						`    deployer.link(${libraryName}, ${contractName});`,
				),
			);
		},
		[],
