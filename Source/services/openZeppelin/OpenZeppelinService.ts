...
export namespace OpenZeppelinService {
	export function projectJsonExists(): boolean {
		return (
			getWorkspaceRoot(true) !== undefined &&
	...
	export async function getCurrentOpenZeppelinVersionAsync(): Promise<string> {
		...
		if (hasProjectJson) {
			const projectMetadata =
				await OpenZeppelinProjectJsonService.getProjectJson();
			currentVersion =
				projectMetadata.openZeppelin.version ||
				Constants.firstOZVersion;
		} else {
			const { defaultValue, userValue } =
				await userSettings.getConfigurationAsync(
	...
	export async function setVersionAsync(
		version: string,
		location: CurrentOpenZeppelinVersionLocation,
	): Promise<void> {
		if (location === CurrentOpenZeppelinVersionLocation.projectJson) {
			await OpenZeppelinProjectJsonService.addVersionToProjectJsonAsync(
				version,
			);
		} else {
	...
	export async function getLatestOpenZeppelinVersionAsync(): Promise<string> {
		const { defaultValue } = await userSettings.getConfigurationAsync(
			Constants.userSettings.ozVersionUserSettingsKey,
		);
		return defaultValue;
	}
	...
	export async function downloadAssetsAsync(
		...
		return Promise.all(
			assets.map(async (asset) => {
				...
				const destinationFilePath = getAssetFullPath(
					destinationFolder || (await getOpenZeppelinFolderPath()),
					asset,
				);
				...
				return download(fileUrl, destinationDirPath, {
	...
	export async function updateProjectJsonAsync(
		...
		let updatedProjectJson =
			await OpenZeppelinProjectJsonService.addVersionToProjectJsonAsync(
				version,
				false,
			);
		updatedProjectJson =
			await OpenZeppelinProjectJsonService.addCategoryToProjectJsonAsync(
				category,
				false,
				updatedProjectJson,
			);
		...
		return OpenZeppelinProjectJsonService.storeProjectJsonAsync(
			updatedProjectJson,
		);
	}
	...
	export function assetHasContracts(asset: IOZAsset): boolean {
		return (
			asset.type === OZAssetType.contract ||
			!!(asset.contracts && asset.contracts.length)
		);
	}
	...
	export function getContractParametersFromAsset(
		asset: IOZAsset,
		contractName: string,
	): string[] {
				...
				return parameters.reduce((arr: string[], param) => {
					if (param.value) {
						if (param.type === "string") {
							...
							param.type.match(
								Constants.validationRegexps.types.simpleArray,
							)
						) {
							arr.push(
								getArrayParameterValue(param.value, param.type),
							);
						} else {
							arr.push(param.value);
						}
	...
	export async function getAssetsStatus(
		assets: IOZAsset[],
	): Promise<{ existing: IOZAsset[]; missing: IOZAsset[] }> {
		...
		return {
			existing: assetsStatuses
				.filter((status) => status.exists === true)
				.map((status) => status.asset),
			missing: assetsStatuses
				.filter((status) => status.exists === false)
				.map((status) => status.asset),
		};
	}

	export async function validateContractsAsync(): Promise<
		OZContractValidated[]
	> {
		const openZeppelinSubfolder = await getOpenZeppelinFolderPath();
		const userProjectMetadata =
			OpenZeppelinProjectJsonService.getProjectJson();
		const ozContractsPaths = getOzContractsFromProjectMetadata(
			openZeppelinSubfolder,
			userProjectMetadata,
		);
		...
		for (const ozContractPath of ozContractsPaths) {
			...
			if (isFileExists(ozContractPath)) {
				const originalHash = getOriginalHash(
					ozContractPath,
					openZeppelinSubfolder,
					userProjectMetadata,
				);
				...
				validatedContract = new OZContractValidated(
					ozContractPath,
					true,
					isHashValid,
				);
	...
	export async function updateOpenZeppelinContractsAsync(
		...
		Telemetry.sendEvent(
			"OpenZeppelinService.updateOpenZeppelinContractsAsync.started",
		);
		const userTmpFolderPath = path.join(
			userWorkspace,
			`${userTmpFolder}${Date.now()}`,
		);
		const tempNewOzFolder = path.join(userTmpFolderPath, "new");
		const tempOldOzFolder = path.join(userTmpFolderPath, "old");
		const tempNewOzContractsFolder = path.join(
			tempNewOzFolder,
			openZeppelinFolderName,
		);

		const currentAssets = await getAllDownloadedAssetsAsync();
		const { isDownloadSucceed, newAssets } =
			await downloadNewVersionOfAssetsAsync(
				currentAssets,
				newManifest,
		...
		try {
			...
			await createNewProjectJsonAsync(
		...
		await moveFolderAsync(
			await getOpenZeppelinFolderPath(),
			tempOldOzFolder,
		);
		...
		Telemetry.sendEvent(
			"OpenZeppelinService.updateOpenZeppelinContractsAsync.finished",
		);
	}
	...
	export async function mergeAssetsWithExisting(
		currentAssets: IOZAsset[],
	): Promise<IOZAsset[]> {
		const assetsWithParameters =
			await OpenZeppelinService.getAssetsWithParameters();

		return currentAssets.map((asset) => {
			if (asset.requiredParameters) {
				const assetWithParameters = assetsWithParameters.find(
					(element) => element.id === asset.id,
				);
				if (
					assetWithParameters &&
					assetWithParameters.requiredParameters
				) {
					for (const [
						currentContractName,
						currentContractParameters,
					] of Object.entries(asset.requiredParameters)) {
						const existedContractParameters =
							assetWithParameters.requiredParameters[
								currentContractName
							];
							...
							for (const parameter of currentContractParameters) {
								const param = existedContractParameters.find(
									(existedContractParameter) =>
										existedContractParameter.name ===
											parameter.name &&
										existedContractParameter.type ===
											parameter.type,
								);
								if (param) {
...
function getOriginalHash(
	...
	const assetName = path
		.relative(openZeppelinSubfolder, ozContractPath)
		.replace(/\\/g, "/");
	const originalAsset = userProjectMetadata.openZeppelin.assets.find(
		(i) => i.name === assetName,
	);
...
async function moveFolderAsync(
	folderPath: string,
...
async function moveProjectJsonAsync(
	nonUserProjectFolder: string,
	folderIsDestination: boolean,
): Promise<void> {
	const userProjectJsonPath =
		OpenZeppelinProjectJsonService.getProjectJsonPath();
...
async function createNewProjectJsonAsync(
	version: string,
	assets: IOZAsset[],
	folder: string,
): Promise<void> {
...
async function downloadNewVersionOfAssetsAsync(
	...
	const assetIdsToDownload = newManifest
		.getAssets()
		.filter((newAsset) =>
			assets.some((asset) => asset.name === newAsset.name),
	...
	const newAssets =
		newManifest.collectAssetsWithDependencies(assetIdsToDownload);
	const sourceUrl = newManifest.getBaseUrlToContractsSource();

	const downloadResult = await OpenZeppelinService.downloadAssetsAsync(
		sourceUrl,
		newAssets,
		true,
		toFolder,
	);
	const isDownloadFailed = downloadResult.some(
		(result) => result.state === PromiseState.rejected,
	);
