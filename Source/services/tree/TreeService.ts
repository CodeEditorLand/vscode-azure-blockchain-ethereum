...
import { type IExtensionView, ViewItemFactory } from "../../ViewItems";
...
export class ExtensionTreeService implements TreeDataProvider<IExtensionView> {
	...
	public getTreeItem(
		element: IExtensionView,
	): IExtensionItem | Promise<IExtensionItem> {
		return element.getTreeItem();
	}

	public getChildren(
		element?: IExtensionView,
	): ProviderResult<IExtensionView[]> {
		if (!element) {
			return TreeManager.getItems().map((item) =>
				ViewItemFactory.create(item),
			);
		}
