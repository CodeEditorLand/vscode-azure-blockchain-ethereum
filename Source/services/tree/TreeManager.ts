...
class ExtensionTreeManager {
	...
	public loadState(): Service[] {
		...
		const store =
			...
			this.globalState.get(
				Constants.globalStateKeys.serviceResourceKey,
				undefined,
			);
		if (store) {
			try {
				...
				if (Array.isArray(obj)) {
					items = obj
						...
						.filter(
							(child) => child instanceof Service,
						) as Service[];
				}
			} catch (error) {
	...
	public getItems(): Service[] {
		...
		if (result.length === 0) {
			Telemetry.sendEvent(
				"TreeManager.getItems.returnDefaultCommandsItems",
			);
	...
	private fillDefaultTypes(items: Service[]): Service[] {
		let azureBlockchainService = items.find(
			(item) => item instanceof AzureBlockchainService,
		);
...
function defaultCommandsItems(): Command[] {
	return [
		new Command(
			"Connect to network",
			"azureBlockchainService.connectProject",
		),
		new Command(
			"Create a new network",
			"azureBlockchainService.createProject",
		),
	];
}
