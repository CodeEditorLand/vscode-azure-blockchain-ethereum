...
class ExtensionContractDB {
	...
	public async initialize(adapterType: AdapterType): Promise<void> {
		...
		if (!this.adapter) {
			Telemetry.sendEvent("ContractDB.initialize.unknownAdapterType", {
				adapterType,
			});
			return;
		}
	...
	public async getContractInstance(
		...
		return (
			this.adapter &&
			(await this.adapter.getContractInstance(contractName, instanceId))
		);
	}
	...
	public async updateContracts(): Promise<void> {
		if (this.adapter) {
			const contracts: ContractInstance[] =
				await this.adapter.getChangedContractInstances();
			const contractNames = contracts
				.map((contract) => contract.contractName)
				.filter(
					(contractName, index, arr) =>
						arr.indexOf(contractName) === index,
				);
