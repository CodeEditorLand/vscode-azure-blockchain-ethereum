...
export class InMemoryAdapter extends AbstractAdapter {
	...
	public async initialize(): Promise<void> {
		const [contracts, networkMaps] =
			await this.getContractsAndNetworksMap();

		contracts.forEach((contract) => {
			this.db[contract.contractName] =
				this.getHistoryFromCompiledContract(contract, networkMaps);
		});
	}

	public async getContractInstances(
		contractName: string,
	): Promise<ContractInstanceWithMetadata[]> {
	...
	public async getContractInstance(
		...
		return history.find(
			(contractInstance) => contractInstance.id === instanceId,
		);
	}

	public async getChangedContractInstances(): Promise<
	...
	> {
		...
		contracts.forEach((contract) => {
			...
			const currentHistory = this.getHistoryFromCompiledContract(
				contract,
				networkMaps,
			);
			const newInstances = currentHistory.filter((contractInstance) => {
				const index = history.findIndex((historyContractInstance) => {
					return (
						historyContractInstance.address ===
	...
	private async getContractsAndNetworksMap(): Promise<
		[Contract[], NetworkMap[]]
	> {
	...
	private getHistoryFromCompiledContract(
		...
		return Object.keys(contract.networks).map((networkKey) => {
			const foundNetwork = networkMaps.find(
				(item) => item.network.id === networkKey,
			);
			if (foundNetwork) {
				return new ContractInstanceWithMetadata(
					contract,
					foundNetwork.network,
					foundNetwork.provider,
				);
			} else {
				return new ContractInstanceWithMetadata(
					contract,
