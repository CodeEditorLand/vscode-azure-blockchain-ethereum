...
import { TruffleConfiguration, getWorkspaceRoot } from "../../helpers";
import { Contract } from "./Contract";

export namespace ContractService {
	export function getContractNameBySolidityFile(
	...
	export async function getCompiledContractsMetadata(): Promise<Contract[]> {
		const contractPaths =
			await getCompiledContractsPathsFromBuildDirectory();
		const contractsMetadata = contractPaths.map((contractPath) =>
			getCompiledContractMetadataByPath(contractPath),
		);
	...
	export async function getDeployedBytecodeByAddress(
		...
		if (!response || (response && response.error)) {
			...
			throw new Error(
				`getDeployedBytecodeByAddress failed. ${errorMessage}`,
			);
		}
	...
	function getCompiledContractMetadataByPath(
		contractPath: string,
	...
	async function getCompiledContractsPathsFromBuildDirectory(): Promise<
		string[]
	> {
		...
		if (!fs.pathExistsSync(buildDir)) {
			throw new Error(
				Constants.errorMessageStrings.BuildContractsDirDoesNotExist(
		...
		return fs
			...
			.filter(
				(file) =>
					path.extname(file) === Constants.contractExtension.json,
			)
