...
export namespace NetworkService {
	...
	export async function getNetworkMaps(
	...
	export async function getNetworkMapByNetworkName(
		...
		const truffleConfig = new TruffleConfig(truffleConfigPath);
		const truffleNetworks = truffleConfig.getNetworks();
		const truffleNetwork = truffleNetworks.find(
	...
	export async function getNetworkIdByHost(
		host: string,
		...
		return {
			...
			provider: host
				? {
						...
						options: {
							mnemonic:
								options.provider && options.provider.mnemonic,
						},
					}
				: null,
		};
	...
	function getTruffleNetworkHost(
		networkOptions: TruffleConfiguration.INetworkOption,
	): string {
		return (
			`${networkOptions.provider ? networkOptions.provider.url : ""}` ||
			`${networkOptions.host ? networkOptions.host : ""}${
				networkOptions.port ? ":" + networkOptions.port : ""
			}`
		);
	}
}
