...
export abstract class MnemonicNetworkNode extends NetworkNode {
	public async getTruffleNetwork(): Promise<TruffleConfiguration.INetwork> {
		...
		const { fs, fsPackageName, hdwalletProvider } =
			Constants.truffleConfigRequireNames;
		...
		try {
			...
			if (!targetURL) {
				window.showInformationMessage(
					Constants.informationMessage.networkIsNotReady(
	...
	private async getMnemonic(): Promise<{ mnemonic: string; path: string }> {
		...
		const savedMnemonics = MnemonicRepository.getExistedMnemonicPaths().map(
			(path) => {
