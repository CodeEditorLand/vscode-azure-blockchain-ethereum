...
import type { IDeployDestination } from "../IDeployDestination";
import { ItemType } from "../ItemType";
import type { AzureBlockchainNetworkNode } from "./AzureBlockchainNetworkNode";
...
export class AzureBlockchainProject extends Project {
	...
	public async getDeployDestinations(): Promise<IDeployDestination[]> {
		...
		this.getChildren().forEach((member) =>
			transactionNodes.push(
				...(member.getChildren() as AzureBlockchainNetworkNode[]),
			),
		);
	...
	private async getNetworkNode(
		deployName: string,
		node: AzureBlockchainNetworkNode,
