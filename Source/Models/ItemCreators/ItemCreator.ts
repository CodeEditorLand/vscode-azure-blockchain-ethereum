...
import type { IExtensionItem } from "../TreeItems";
...
export abstract class ItemCreator {
	...
	protected getAdditionalConstructorArguments(_obj: {
		[key: string]: any;
	}): any[] {
		return [];
	}
		...
		requiredFields.forEach((item) => {
			...
			if (field === undefined || field === null) {
				Telemetry.sendException(
					new Error(`Missed required field ${item.fieldName}.`),
				);
				throw new Error(
			...
			if (
				(item.type === "array" && Array.isArray(field)) ||
				typeof field === item.type
			) {
				return;
			}
			...
			Telemetry.sendException(
				new Error(
					`Required field ${item.fieldName} should be type ${item.type}`,
				),
			);
			throw new Error(
				`Required field ${item.fieldName} should be type ${
					item.type
				}. JSON: ${JSON.stringify(obj)}`,
			);
		});
