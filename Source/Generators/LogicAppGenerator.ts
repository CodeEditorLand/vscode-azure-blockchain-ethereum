...
export class LogicAppGenerator {
	...
	public async generateMicroservicesWorkflows(
		filePath: Uri | undefined,
	): Promise<void> {
		return this.generateWorkflows("Service", filePath);
	}

	public async generateDataPublishingWorkflows(
		filePath: Uri | undefined,
	): Promise<void> {
	...
	public async generateReportPublishingWorkflows(
		filePath: Uri | undefined,
	): Promise<void> {
	...
	private async generateWorkflows(
		...
		if (filePath) {
			...
			if (workflowType === "Service") {
				...
				picks = [{ label: "Logic App" }, { label: "Flow App" }];
			}
			...
			readJson(
				...
				{ encoding: "utf8" },
				async (err2: Error, contents: any) =>
					await this.handleContractJsonFile(
						err2,
						contents,
						dirPath,
						contractAddress,
	...
	private async iterateFilesInDirectory(
		err: Error,
		files: string[],
		dirPath: string,
		workflowType: string,
	...
	private async createLogicAppFromAbi(
		...
		if (workflowType === "Service") {
			...
			generator =
				new Nethereum.Generators.DataWorkflow.DataWorkflowProjectGenerator(
					buildContract(JSON.stringify(contract.abi)),
					contract.contractName,
					contract.bytecode,
					contract.contractName,
			...
			generator =
				new Nethereum.Generators.MessagingWorkflow.MessagingWorkflowProjectGenerator(
					buildContract(JSON.stringify(contract.abi)),
					contract.contractName,
					contract.bytecode,
					contract.contractName,
			...
			generator =
				new Nethereum.Generators.ReportingWorkflow.ReportingWorkflowProjectGenerator(
					buildContract(JSON.stringify(contract.abi)),
					contract.contractName,
					contract.bytecode,
					contract.contractName,
		...
		if (generator) {
			...
			throw new Error(
				"workflowType does not match any available workflows",
			);
		}
	}
	...
	private async selectSubscriptionAndResourceGroup(): Promise<
		[SubscriptionItem, ResourceGroupItem]
	> {
	...
	private async getOrSelectSubscriptionItem(): Promise<SubscriptionItem> {
		return await showQuickPick(await this.loadSubscriptionItems(), {
	...
	private async loadSubscriptionItems(): Promise<SubscriptionItem[]> {
		...
		if (subscriptionItems.length === 0) {
			throw new Error(
				"No subscription found, click an Azure account at the \
