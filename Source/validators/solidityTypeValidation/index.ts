	...
	switch (type) {
		...
		case Constants.solidityTypes.bool:
			return value === "true" || value === "false"
				? undefined
				: Constants.validationMessages.valueShouldBeBool;
		case Constants.solidityTypes.address:
			return value.match(
				Constants.validationRegexps.types.solidityAddress,
			)
				? undefined
...
function validateNumber(value: string, type: string): string | undefined {
	...
	if (type.match(Constants.validationRegexps.types.solidityUint)) {
		...
		const maxUint =
			bigIntMath.pow(BigInt(2), Number.parseInt(pow, 10)) - BigInt(1);
		const valueAsNumber = BigInt(Number.parseInt(value, 10));
		if (valueAsNumber < BigInt(0) || valueAsNumber > maxUint) {
			return Constants.validationMessages.valueShouldBePositiveAndCanSafelyStoreUpToBits(
				pow,
			);
		}
	}
	...
	if (type.match(Constants.validationRegexps.types.solidityInt)) {
		...
		const maxInt =
			bigIntMath.pow(BigInt(2), Number.parseInt(pow, 10) - 1) - BigInt(1);
		const minInt = -maxInt;
		const valueAsNumber = BigInt(Number.parseInt(value, 10));
		if (valueAsNumber < minInt || valueAsNumber > maxInt) {
			return Constants.validationMessages.valueCanSafelyStoreUpToBits(
				pow,
			);
		}
	}
...
function validateSimpleArray(
	...
	if (invalidValues.length > 0) {
		return Constants.validationMessages.arrayElementsShouldBeValid(
			elementsType,
		);
	}
